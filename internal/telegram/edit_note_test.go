...
func TestSavedMarkup_EditOnlyWhenLive(t *testing.T) {
	tb := newTestBot(t)

	live := tb.savedMarkup(5, false, false)
	if !markupHasButton(live, tb.msg.EditBtn) {
		t.Error("live note should offer the Edit button")
	}
	discarded := tb.savedMarkup(5, true, false)
	if markupHasButton(discarded, tb.msg.EditBtn) {
		t.Error("discarded note must NOT offer the Edit button")
	}
}
